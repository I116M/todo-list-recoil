...
const TodoItem = ({item}) => {
    ...
    const editItemText = ({target: {value}}) => {
        ...
        const newList = todoList.map((listItem) => (listItem.id === item.id ? {...listItem, text: value} : listItem));
        setTodoList(newList);
    };

    const toggleItemCompletion = () => {
        const newList = todoList.map((listItem) => (listItem.id === item.id ? {...listItem, isComplete: !item.isComplete} : listItem));
